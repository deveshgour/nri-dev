					...
					<div class="dropdown moodDropdoown notification">
						...
						<div class="dis">	<span class="badge"><?php echo count($notification_count); ?></span></div>
							...
							<?php }elseif($noti->status == 1 && $noti->user_id == $this->session->userdata('userId')['user_id']){
								...
								?>
                                <div class="dis"></div> <span class="badge"><?php echo count($notification_count); ?></span>	</div>				
								 <?php }}} ?>
                        </a>
						<div class="dropdown-menu custom-menus" aria-labelledby="moodDropdoown">
							...
							<?php }elseif($noti->status == 1 && $noti->user_id == $this->session->userdata('userId')['user_id']){
                              $username = $this->Common_model->getsingle("users",array("user_id" => $noti->friend_user_id));
								?>
                               <span class="notification__wrap"><span class="notification__msg"><a href="<?php echo base_url(); ?>friend-user/<?php echo base64_encode($noti->friend_user_id); ?>"><b><?php echo ucwords($username->firstname.' '.$username->lastname); ?></b> Accept your friend request</a></span>
							   <span class="notification__time"><?php echo convert_time($noti->create_date,'F j, Y, g:i a'); ?></span> </span>
							<?php }else{ ?>
							No record found
							<?php }}}else{ ?>
               ...
               <li class="list-inline-item">
                ...
                <li class="list-inline-item">
                    ...
                    <div class="dropdown">
                        ...
                        <a class="dropdown-item" href="<?php echo base_url(); ?>support"><em class="icon icon-message-circle"></em> Support System</a>
