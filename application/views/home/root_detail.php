            ...
            <div class="col-xl-6 col-lg-9">
				...
				<form id="newuploadimage" action="" method="post" enctype="multipart/form-data">
                    ...
                    <div class="feed-box rounded-10">
                        ...
                        <?php  
	    $likeType = $this->Common_model->getsingle('like_root', array('root_id' => $detail->root_id,'status'=>1,'user_id' => $this->session->userdata('userId')['user_id'])); 
		$count_like_one = $this->Common_model->jointwotable('like_root', 'user_id', 'users', 'user_id',array('root_id' => $detail->root_id,'like_root.status'=>1,'users.status'=>2),'users.user_id,users.firstname,users.lastname,like_root.like_root_id'); 
	//echo $this->db->last_query();die;
	$count_like_ss = $this->Common_model->joint_root($detail->root_id);
	
	$totalcommentcount = $this->Common_model->countwhereuser("comment_root",array("root_id" => $detail->root_id));
	?>						
							
                            <div class="d-flex justify-content-between">
                                <div id="likecountbox-<?php echo $detail->root_id; ?>" class="font-sm" data-likecount="<?php echo COUNT($count_like_one); ?>" >  <span><?php echo COUNT($count_like_one); ?> people like this</span><?php /* ?>Kavi Singh and 200 other like_govd<?php */ ?></div>
                               <?php if($totalcommentcount != "0"){ ?><div class="comments"><span class="totalcmt<?php echo $detail->root_id; ?>"><?php echo $totalcommentcount; ?></span> Comments</div><?php } ?>
                            </div>
                        </div>
                       <div class="feedBottom">
                            <div class="likeShare d-flex align-items-center justify-content-between">
                                <div class="left">
                               <a href="javascript:void(0)" class="link-dark like_root" <?php if($likeType){ ?>style="color:#009688;"<?php } ?> id="like-<?php echo $detail->root_id; ?>" data-status="1" data-type="1" data-author="<?php echo $detail->user_id; ?>" data-postid="<?php echo $detail->root_id; ?>">
			...
			<?php /*if($like_govType){ ?><a class="like_gov_post" id="like_gov-<?php echo $detail->gov_id; ?>" style="color:#009688;" href="javascript:void(0)" data-status="1" data-type="1" data-author="<?php echo $detail->user_id; ?>" data-postid="<?php echo $detail->gov_id; ?>" ><i class="fa fa-thumbs-up"></i>like_gov</a> <?php }else{ ?> <a class="like_gov_post" id="like_gov-<?php echo $detail->gov_id; ?>" href="javascript:void(0)" data-type="1" data-postid="<?php echo $detail->gov_id; ?>" data-author="<?php echo $detail->user_id; ?>" data-status="1"><i class="fa fa-thumbs-up"></i>like_gov</a> <?php } */?>
                            ...
                            <ul class="list-unstyled comment-list commentshow-<?php echo $detail->root_id; ?>">
							<div class="sds<?php echo $detail->root_id; ?>">
							<?php
$getallcomment = $this->Common_model->commentpaginationroot(array("root_id" => $detail->root_id),3,0,"comment_root_id", "desc");
	      ...
	      foreach($getallcomment['rows'] as $allcomment){
			  $getuser = $this->Common_model->getsingle("users",array("user_id" => $allcomment->user_id)); ?>
				...
				<div class="seeComment my-3"><center style="text-align: left;color:black;"><a class="morerootimage" data-postid="<?php echo $detail->root_id; ?>" data-page="1" href="javascript:void(0);">See more comments</a></center></div>
							<?php }}else{ ?>
							...
							<form id="cmtrootform-<?php echo $detail->root_id; ?>" data-postid="<?php echo $detail->root_id; ?>" data-userid="<?php echo $this->session->userdata("userId")['user_id']; ?>" action="" method="post" >
                            <input type="hidden" name="postId" id="postId" value="<?php echo $detail->root_id; ?>" >
							...
							<div class="postComment d-flex align-items-center pt-2">
                                ...
                                <div class="inputBox position-relative">
                                    <input type="text" class="form-control newpostcomment post_comment-<?php echo $detail->root_id; ?>" name="post_comment" id="post_comment" data-cmtid="<?php echo $detail->root_id; ?>" value="" placeholder="Write a public comment">
                                    <button class="border-0 bg-transparent font-sm postVal_<?php echo $detail->root_id; ?>" disabled id="postVal">Post</button>
                                </div>
                            </div>
							</form>
                        </div>
