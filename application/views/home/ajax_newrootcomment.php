     ...
     if(!empty($getallcomment)){
	      foreach($getallcomment['rows'] as $allcomment){
			 ...
			 <li class="d-flex">
									...
									<?php if($allcomment->user_id == $this->session->userdata('userId')['user_id']){ ?><div><a href="javascript:void(0);" class="trashrootcomments<?php echo  $allcomment->comment_root_id; ?>" id="deleteComment" data-commentid="<?php echo  $allcomment->comment_root_id; ?>"><span class="icon-trash-2 trashrootcomment" data-commentid="<?php echo  $allcomment->comment_root_id; ?>" data-postid="<?php echo $post_id; ?>"></span></a></div><?php } ?>
                                </li>
								</span>
         <?php }if($getallcomment['num_rows'] > count($getallcomment['rows'])){ ?>
				<div class="seeComment my-3"><center style="text-align: left;color:black;"><a class="morerootimage" data-postid="<?php echo $allcomment->root_id; ?>" data-page="<?php echo $offset; ?>" href="javascript:void(0);">See more comments</a></center></div>		
				<?php }
	 }else{ ?>
