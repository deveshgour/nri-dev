     ...
     if(!empty($getallcomment)){
	      foreach($getallcomment['rows'] as $allcomment){
			  ...
			  <li class="d-flex">
									...
									<div><a href="javascript:void(0);" id="deleteComment" data-commentid="<?php echo  $allcomment->comment_major_id; ?>"><span class="icon-trash-2 trashmajorcomment" data-commentid="<?php echo  $allcomment->comment_major_id; ?>" data-postid="<?php echo $post_id; ?>"></span></a></div>
                                </li>
								</span>
         <?php }if($getallcomment['num_rows'] > count($getallcomment['rows'])){ ?>
				<div class="seeComment my-3"><center style="text-align: left;color:black;"><a class="moremajorimage" data-postid="<?php echo $allcomment->major_id; ?>" data-page="1" href="javascript:void(0);">See more comments</a></center></div>
							<?php }}else{
	 }
?>
</div>
