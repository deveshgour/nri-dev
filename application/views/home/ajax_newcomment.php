     ...
     if(!empty($getallcomment)){
	      foreach($getallcomment['rows'] as $allcomment){
			 ...
			 <li class="d-flex">
									...
									<div><a href="javascript:void(0);" id="deleteComment" data-commentid="<?php echo  $allcomment->comment_id; ?>"><span class="icon-trash-2 trashcomment" data-commentid="<?php echo  $allcomment->comment_id; ?>" data-postid="<?php echo $post_id; ?>"></span></a></div>
                                </li>
								</span>
         <?php }if($getallcomment['num_rows'] > count($getallcomment['rows'])){ ?>
				<div class="seeComment my-3"><center style="text-align: left;color:black;"><a class="moreimage" data-postid="<?php echo $allcomment->post_id; ?>" data-page="<?php echo $offset; ?>" href="javascript:void(0);">See more comments</a></center></div>		
				<?php }
	 }else{ ?>
